...
.errorhandling = "pass"
    ...
    est_lab_y <- summary(lm(Y ~ X, data = lab))$coefficients[2, 1]
    est_lab_yhat <- summary(lm(Yhat ~ X, data = lab))$coefficients[2, 1]
    est_unlab_yhat <- summary(lm(Yhat ~ X, data = unlab))$coefficients[2, 1]
    n <- nrow(lab)
    N <- nrow(unlab)
    ...
    for (i in 1:B){
        ...
        est_lab_y_boot[i, ] <- lm(Y ~ X, data = boot_lab)$coefficients[2]
        est_lab_yhat_boot[i, ] <- lm(Yhat ~ X, data = boot_lab)$coefficients[2]
        est_unlab_yhat_boot[i, ] <- lm(Yhat ~ X, data = boot_unlab)$coefficients[2]
    }
    ...
    true_beta <- summary(lm(Y ~ X, data = full))$coefficients[2, 1]

    # Classic method
    fit_classic <- lm(Y ~ X, data = lab)
    est_classic <- fit_classic$coefficients[2]
    # Need to use sandwich standard error
    # robust_se <- sqrt(diag(vcovHC(fit_classic, type = "HC1")))
    # se_classic <- robust_se[2]
    ...
    # Imputed method
    fit_imputed <- lm(Yhat ~ X, data = unlab)
    est_imputed <- fit_imputed$coefficients[2]
    # Need to use sandwich standard error
    # robust_se <- sqrt(diag(vcovHC(fit_imputed, type = "HC1")))
    # se_imputed <- robust_se[2]
    ...
    # PPI
    X_lab_tmp <- as.matrix(cbind(1, X_lab))
    Y_lab <- as.matrix(Y_lab)
    Yhat_lab <- as.matrix(Yhat_lab)
    X_unlab_tmp <- as.matrix(cbind(1, X_unlab))
    Yhat_unlab <- as.matrix(Yhat_unlab)

    fit_ppi <- IPD::ppi_plusplus_ols(X_lab_tmp, Y_lab, Yhat_lab, X_unlab_tmp, Yhat_unlab, lhat = 1)
    ...
    # PPI++
    fit_ppi_plus_plus <- IPD::ppi_plusplus_ols(X_lab_tmp, Y_lab, Yhat_lab, X_unlab_tmp, Yhat_unlab)
    ...
    # POP-Inf
    fit_popinf <- pop_M(X_lab, X_unlab, Y_lab, Yhat_lab, Yhat_unlab, alpha = 0.05, method = "ols")
...
fwrite(as.data.frame(result), paste0("./results/ols_", ratio, ".txt"),

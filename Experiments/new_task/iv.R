...
result <- foreach(
i = 1:sim.times, .combine = rbind, .packages = c("POPInf","quantreg", "sandwich", "lmtest", "MASS", "AER"),
.errorhandling = "pass"
) %dopar% {
    r <- 0.8
    data <- sim_data_iv(r = r)
    X_lab = data$X_lab
    ...
    Z_lab = data$Z_lab
    Z_unlab = data$Z_unlab

    full <- data.frame(X = c(unlist(X_lab), unlist(X_unlab)), Y = c(unlist(Y_lab), unlist(Y_unlab)), Z = c(unlist(Z_lab), unlist(Z_unlab)))
    ...
    lab <- data.frame(Y_lab, Yhat_lab, X_lab, Z_lab)
    colnames(lab) <- c("Y", "Yhat", "X", "Z")
    unlab <- data.frame(Yhat_unlab, X_unlab, Z_unlab)
    colnames(unlab) <- c("Yhat", "X", "Z")
    est_lab_y <- summary(ivreg(Y ~ X | Z, data = lab))$coefficients[2, 1]      
    est_lab_yhat <- summary(ivreg(Yhat ~ X | Z, data = lab))$coefficients[2, 1] 
    est_unlab_yhat <- summary(ivreg(Yhat ~ X | Z, data = unlab))$coefficients[2, 1] 
    n <- nrow(lab)
    N <- nrow(unlab)
    ...
    for (i in 1:B){
        ...
        est_lab_y_boot[i, ] <- ivreg(Y ~ X | Z, data = boot_lab)$coefficients[2]
        est_lab_yhat_boot[i, ] <- ivreg(Yhat ~ X | Z, data = boot_lab)$coefficients[2]
        est_unlab_yhat_boot[i, ] <- ivreg(Yhat ~ X | Z, data = boot_unlab)$coefficients[2]
    }
    ...
    true_beta <- summary(ivreg(Y ~ X | Z, data = full))$coefficients[2, 1]
    ...
    # Imputed method
    est_imputed <- summary(ivreg(Yhat ~ X | Z, data = unlab))$coefficients[2, 1]
...
fwrite(as.data.frame(result), paste0("./results/iv_", ratio, ".txt"),
